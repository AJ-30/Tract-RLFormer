...
from rl_environments.rl import RLAlgorithm
...
#             nn.TransformerEncoderLayer(d_model=hidden_dim, nhead=num_heads, dim_feedforward=hidden_dim*4, dropout=dropout_prob),
#             num_encoder_layers
#         )
...
#         # Assuming state has shape (batch_size, state_dim)
#         batch_size = state.size(0)

#         # Embedding the state
#         p = torch.relu(self.embedding(state))  # Shape: (batch_size, hidden_dim)
#         p = torch.relu(self.l2(p))

#         # Reshaping the input to be suitable for Transformer Encoder (sequence length of 1)
#         p = p.unsqueeze(1)  # Shape: (batch_size, seq_len=1, hidden_dim)

#         # Transformer Encoder
#         p = self.encoder_layers(p)  # Shape: (batch_size, seq_len=1, hidden_dim)

#         # Removing the sequence length dimension before final output
#         p = p.squeeze(1)  # Shape: (batch_size, hidden_dim)

#         # Output layer
#         p = self.output_activation(self.output_layer(p))  # Shape: (batch_size, action_dim)
...
#         # Embedding the state
#         p = self.embedding(state)  # Shape: (batch_size, hidden_dim)
    ...
    def load(self, path: str, filename: str):
        ...
        -----------
            ...
            filename: string
                ...
                will be appended
        """
        print(pjoin(path, filename + '_actor.pth'))
