...
import pickle
# pickleFile = open('/datasets/tracto_ashutosh/traj_Fornix/forward/ttl_traj_list_1_4921.pkl','rb')
...
#         rng = np.random.randint(1,8756)
#         state = env.reset(rng,rng+1)#BY ASHUTOSH
...
#     #IF MULTI_TRAJ--------------------------------------------
#     # states = torch.from_numpy(init_states).reshape(5, state_dim).to(device=device, dtype=torch.float32)
#     # actions = torch.zeros((0, act_dim), device=device, dtype=torch.float32)
#     # actions = torch.cat([torch.from_numpy(init_actions[:4]).reshape(4,act_dim).to(device=device, dtype=torch.float32) , actions], dim=0)
#     # rewards = torch.zeros(0, device=device, dtype=torch.float32)
#     # rewards = torch.cat([torch.from_numpy(init_rewards[:4]).reshape(4).to(device=device, dtype=torch.float32) , rewards], dim=0)
#     # target_return = torch.tensor(target_return, device=device, dtype=torch.float32)
#     #IF MULTI_TRAJ--------------------------------------------
...
#         state, reward, done, _ = env.step(action)
#         if isinstance(env, TrackingEnvironment):#ASHUTOSH
#             state = state.detach().cpu().numpy()#ASHUTOSH
#             reward = reward[0]#ASHUTOSH
...
#             # print('--------------------------------------------------')
#             # print(done)
#             # print(fl[7]['observations'][0][:2])
#             # print(fl[7]['actions'][0])
#             # print(states[-2][:2])
#             # print(action,'\n\n\n')
#             # print('--------------------------------------------------')
#             break
...
def evaluate_episode_rtg(
    ...
    if isinstance(env, TrackingEnvironment):
        ...
        state = env.reset(rng,rng+4)#BY ASHUTOSH
        state = state.detach().cpu().numpy()
        #IF FROM TRAJECTORY---------
        # state = init_state #CAN'T DO LIKE THIS
        #IF FROM TRAJECTORY---------
    ...
    rewards = torch.zeros(0, device=device, dtype=torch.float32)
    # print('herenk',actions.shape, rewards.shape)
    ...
    # print('herenk2222',actions.shape, rewards.shape, target_return.shape, timesteps.shape)
    ...
    for t in range(max_ep_len):
        ...
        # print('act-herererere',action.shape)
        # print('act-herererere',actions.shape)
        actions[:,-1,:] = action
        action = action.detach().cpu().numpy()
        ...
        # print('jwkcsbd---12qwe1',states.shape, pred_return.shape)
        target_return = torch.cat(
